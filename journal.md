Phase 1: Explain the syntax difference between the function declaration (calculatePrice) and the function expression (getTax)?
The function declaration uses a variable which can be changed. And the function expression uses a constant which cannot be changed. It also can be called no matter if it's above or below the code. 

Phase 2:
In this phase I had to research just exactly how to use the get tax variable in this phase. But once I did that I understand how scope works. This variable only works within that function. When I tried to log default tax rate outside the function it gave a nan error. 


Phase 3:
In this phase I used the consise arrow syntax which made the code a lot easier to understand by simplifying the syntax. 
Do I have to call the phase 1 console.log everytime or does it just stay in the code to appear on the console? So that when I refresh it it has the same results everytime. 

Phase 4:
Why is applyDiscount considered a “Higher-Order Function”? What role did studentDiscount and seniorDiscount play? What function would you have to write if you wanted to apply no discount?

It is considered a high order function because it takes on another argument. Student discount and senior discount played the role of the argument. I don't know quite yet what to write to apply no discount. Unless you just didn't use the functions senior discount and student discount. 

Phase 5:
In this phase I created a function and broke down the steps to the code inside which made it a lot easier to comprehend and understand what I was doing. 


